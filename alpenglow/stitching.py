...
def apply_shift(image1, image2, shift, margin=100):
    ...
    if margin > 0:
        ...
        if shift[1] >= 0:
            ...
            fade1[:, abs(int(shift[1])):] = image1[overlap - margin:overlap, :int(shift[1])] * np.arange(0, 1,
                                                                                                         (1 / margin))[
                                                                                               :, np.newaxis]
...
def stitch_zstack(images_1, images_2, nchan, chan=0, current_stack=None, out_path=None):
    ...
    if nchan == 1:
        return output_zstack(images_1, images_2, coef, shift, chop_index, current_stack=current_stack,
                             out_path=out_path)
